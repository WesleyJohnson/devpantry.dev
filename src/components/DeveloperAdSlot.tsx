...
export function DeveloperAdSlot({ variant = 'sidebar', className }: DeveloperAdSlotProps) {
  if (variant === 'banner') {
    return (
      <div
        className={clsx(
          'flex items-center justify-between gap-3 rounded-lg border border-dashed border-zinc-300 px-4 py-2.5 text-xs text-zinc-500 dark:border-zinc-800 dark:text-zinc-500',
          className,
        )}
      >
  ...
  return (
    <div
      className={clsx(
        'flex h-[100px] w-full max-w-[130px] flex-col items-center justify-center gap-1 rounded-lg border border-dashed border-zinc-300 p-2 text-center text-zinc-500 dark:border-zinc-800 dark:text-zinc-500',
        className,
      )}
    >
